...
void loop(){
           while (100000 > (millis()-StartTime)){ // время полёта
             ...
             if (millis() - preTime[0] > 50)               // ОПРОС ДАТЧИКОВ каждые 50мс
              info();
             
             if(millis()-StartTime > 4000 && velocity < 10 && firstSave == 0){//ОснСистСпасения. Условие спасения по скорости + сообщение в лог
             logfile  =  SD.open(filename, FILE_WRITE);
             ...
             if((millis()-StartTime) > 10500 && firstSave == 0){   // ОснСистСпасения. Условие спасения по времени
              logfile  =  SD.open(filename, FILE_WRITE);
              logfile.print("TimeSave");
               logfile.print(millis());
               bufTime = millis();
               Serial.println("StartSaveSystemTime");
             ...
             if (secondSave == 0 && firstSave == 1 && digitalRead(DownPin) == LOW && (millis()-bufTime > 2500)){//доп. система спасения по концевику
             ...
             logfile.println("StartReserveSaveSystemEndcap");
              ...
              if (secondSave == 0 && firstSave == 1 && velocity < (-15) && (millis()-bufTime > 3000)){//доп. система cпасения по скорости
